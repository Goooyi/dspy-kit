...
from dspy_kit import (
    PrecisionGrader, RecallGrader, F1Grader, AccuracyGrader,
    ClassificationMetricsGrader, IntentClassificationGrader,
    create_intent_classifier_grader, create_classification_grader
)
...
class CustomerSupportIntentClassifier(dspy.Module):
    def __init__(self):
        self.classifier = dspy.ChainOfThought(
            "customer_query -> intent",
            signature="Given a customer support query, classify the intent."
        )
...
unknown_example = dspy.Example(
    customer_query="Test query", 
    expected_intent="unknown_intent"
).with_inputs("customer_query")
...
general_grader = create_classification_grader(
    labels=["billing", "technical_support", "cancellation", "product_inquiry"],
    average="weighted"
)
    ...
    def __init__(self):
        self.intent_grader = create_intent_classifier_grader(
            intents=["billing", "technical_support", "cancellation", 
                    "product_inquiry", "general_inquiry", "escalation"],
            average="weighted"
        )
