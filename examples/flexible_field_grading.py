...
def example_1_basic_flexibility():
    ...
    different_grader = SemanticSimilarityGrader(pred_field="answer", ideal_field="gold_standard", threshold=0.8)
...
def example_2_dspy_program_integration():
    ...
    class CustomQASystem(dspy.Module):
        def __init__(self):
            super().__init__()
            self.qa = dspy.ChainOfThought("question -> detailed_answer")  # type: ignore

        def forward(self, question):
            result = self.qa(question=question)
            # Return with custom field name
            return dspy.Prediction(question=question, detailed_answer=result.detailed_answer, confidence=0.85)
...
def example_3_customer_support_scenario():
    ...
    support_grader = create_customer_support_grader(
        response_field="agent_response", query_field="customer_inquiry", reference_field="ideal_agent_response"
    )

    # Evaluate
    example = support_data[0]
    prediction = {"agent_response": example["agent_response"], "confidence": 0.9, "escalation_needed": False}
...
def example_4_optimization_capability():
    ...
    grader = FactualAccuracyGrader(pred_field="model_output", ideal_field="ground_truth", threshold=0.8)
...
def example_5_handling_edge_cases():
    ...
    grader = SemanticSimilarityGrader(pred_field="answer", ideal_field="expected")

    # Test with different object types
    test_cases = [
        # Case 1: Normal dict
        ({"expected": "Python is a programming language"}, {"answer": "Python is a coding language"}),
        # Case 2: Object with attributes
        (type("Example", (), {"expected": "Python is great"})(), type("Pred", (), {"answer": "Python is awesome"})()),
        # Case 3: Missing fields (graceful fallback)
        ({"some_other_field": "irrelevant"}, {"different_field": "Python is cool"}),
        # Case 4: Mixed types
        ({"expected": "Python programming"}, "Python is a language"),  # String fallback
    ]
...
def example_6_comparison_with_traditional_dspy():
    ...
    different_data = {"gold_answer": "AI is machine intelligence", "category": "tech"}

    different_pred = {"generated_text": "AI means artificial intelligence", "model": "gpt-4"}

    different_grader = SemanticSimilarityGrader(pred_field="generated_text", ideal_field="gold_answer")
